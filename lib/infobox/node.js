define(["moment", "numeral", "tablesort", "tablesort.numeric"],
  ...
  var modelhash = model.split("").reduce(function (a, b) { a = ((a << 5) - a) + b.charCodeAt(0); return a & a }, 0)
    ...
    if (o.thumbnail) {
      ...
      content.onerror = function () {o.thumbnail.replace("{MODELHASH}", "unknown")}
  ...
  return function(config, el, router, d) {
    ...
    try {
      if (config.hwImg)
        config.hwImg.forEach( function (hwImg) {
          ...
          // el.appendChild(showNodeImg(hwImg, d.nodeinfo.hardware.model))
        })
    }
    catch(err) {
    ...
    attributeEntry(attributes, "Clients im Mesh", showMeshClients(d))
